...
STEP=${1:-120}
case "$STEP" in
  120) NAME=t2T_120;   REPO=oerdogan/qwen3-30b-t2-treatment-lora-step120 ;;
  200) NAME=t2T_final; REPO=oerdogan/qwen3-30b-t2-treatment-lora-step200 ;;
  *) echo "unsupported treatment step $STEP (only 120/200 survive)"; exit 1 ;;
esac
EFF=/scratch/omeerdogan23/erlm/.research/EfficientRLM
PRL=/scratch/omeerdogan23/erlm/prime-rl
ADIR=$EFF/scripts/offline_eval/adapters/$NAME
...
p = snapshot_download("$REPO",
                      local_dir="/scratch/tmp/omeerdogan23/canary/${NAME}_repo")
print(p)
PY
  INNER=$(dirname "$(find /scratch/tmp/omeerdogan23/canary/${NAME}_repo -name adapter_config.json | head -1)")
...
GPUS=$GPUS PORT=$PORT TP=4 GPU_MEM_UTIL=0.92 bash 00_serve.sh > /tmp/canary_serve_${NAME}_$PORT.log 2>&1 &
SRV=\$!
for i in \$(seq 1 90); do curl -s localhost:$PORT/health >/dev/null && break; sleep 10; done
curl -s localhost:$PORT/health >/dev/null || { echo SERVE_FAILED; tail -5 /tmp/canary_serve_${NAME}_$PORT.log; kill \$SRV; exit 2; }
BASE_URL="http://localhost:$PORT/v1" POLICIES_OVERRIDE="$POL" SUITE_FILTER=codeqa N_BC=0 \
  OUT=$EFF/outputs/canary_t2T_$STEP bash 01_run_evals.sh
...
import glob, json, os
fs = sorted(glob.glob("$EFF/outputs/canary_t2T_$STEP/codeqa/*/**/results.jsonl", recursive=True), key=os.path.getmtime)
fs = [f for f in fs if "$NAME" in f]
...
line = f"| $STEP-TREATMENT | {acc:.3f} | {sum(subs)/n:.1f} | {zero:.2f} | {cap:.2f} | {fin:.2f} | n={n} |"
