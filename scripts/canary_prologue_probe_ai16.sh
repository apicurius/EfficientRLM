#!/bin/bash
# Exploratory: CodeQA under the DOMAIN-NEUTRAL prologue (the template new
# LongBench domains use) instead of the preserved code-specific one.
# Deconfounds prologue wording from domain in the cross-domain propensity
# comparison. Pre-registered canary rows keep the original prologue untouched.
# Usage: canary_prologue_probe_ai16.sh <policy>   (base | t2T_final | ...)
set -euo pipefail
POL=${1:?usage: canary_prologue_probe_ai16.sh <policy>}
...
GPUS=$GPUS PORT=$PORT TP=4 GPU_MEM_UTIL=0.92 bash 00_serve.sh > /tmp/probe_serve_neutral_${SAFE}_$PORT.log 2>&1 &
SRV=\$!
for i in \$(seq 1 90); do curl -s localhost:$PORT/health >/dev/null && break; sleep 10; done
curl -s localhost:$PORT/health >/dev/null || { echo SERVE_FAILED; tail -5 /tmp/probe_serve_neutral_${SAFE}_$PORT.log; kill \$SRV; exit 2; }
# env-args built in python so the neutral prologue text comes from the env
# module itself (no duplicated prompt text in this script)
EA=\$(python - <<'PYEOF'
import json
from longbench_codeqa.env import _prologue_for, LONGBENCH_CODE_DOMAIN
print(json.dumps({
    "max_iterations": 20, "sub_max_tokens": 4096,
    "user_prologue": _prologue_for(LONGBENCH_CODE_DOMAIN),
}))
PYEOF
)
vf-eval longbench_codeqa --env-dir-path $EFF/rlm/training/environments \
  ...
  --env-args "\$EA" \
  --save-results --output-dir $EFF/outputs/probe_neutralprologue_$SAFE/codeqa_neutral/${SAFE} --disable-tui
kill \$SRV 2>/dev/null || true
INNER

$PRL/.venv/bin/python - <<PY
import glob, json, os
fs = sorted(glob.glob("$EFF/outputs/probe_neutralprologue_$SAFE/codeqa_neutral/**/results.jsonl", recursive=True), key=os.path.getmtime)
...
line = f"| CODEQA-NEUTRALPROLOGUE {('$POL').split('/')[-1]} | {acc:.3f} | {sum(subs)/n:.1f} | {zero:.2f} | - | {fin:.2f} | n={n} |"
