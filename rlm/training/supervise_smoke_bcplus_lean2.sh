#!/bin/bash
# Supervise the BC+ lean smoke from inside a tmux session on the login node.
...
# from the latest COMPLETE checkpoint. Drops SMOKE_DONE on clean completion,
# SMOKE_FAILED after 3 consecutive fast failures.
set -u

EFF=/scratch/omeerdogan23/erlm/.research/EfficientRLM
OUT="$EFF/outputs/smoke-qwen3-30b-bcplus-lean2-winaligned-60step"
LOG="${SMOKE_LOG:?set SMOKE_LOG to the console log path}"
DONE="$OUT/SMOKE_DONE"
FAILED="$OUT/SMOKE_FAILED"
HOLDER=1263387
MAX_STEP=60
...
while :; do
  ...
  echo "[supervise] $(date '+%F %T') lean smoke launching (resume=${STEP:-fresh})" >> "$LOG"
  START=$(date +%s)
  srun --overlap --jobid="$HOLDER" --gres=gpu:rtx_a6000:8 -n1 \
    --cpus-per-task=48 --mem=0 --time=23:00:00 bash -l \
    "$EFF/rlm/training/run_smoke_bcplus_lean2.sh" "${RESUME_ARGS[@]}" >> "$LOG" 2>&1
  RC=$?
  ELAPSED=$(( $(date +%s) - START ))
  if [ "$RC" -eq 0 ]; then
    if [ -d "$OUT/run_default/rollouts/step_$MAX_STEP" ]; then
      echo "[supervise] $(date '+%F %T') lean smoke COMPLETED rc=0 at step $MAX_STEP" >> "$LOG"
      touch "$DONE"
      exit 0
    fi
