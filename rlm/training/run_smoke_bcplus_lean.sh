#!/bin/bash
# BC+-only LEAN 60-step smoke on the EfficientRLM tree (group_size 8,
...
CFG=$EFF/rlm/training/configs/smoke-qwen3-30b-bcplus-lean-60step.toml
mkdir -p $EFF/outputs
...
echo "[smoke] ===== START bcplus-lean-60step $(date +%F_%H:%M:%S) host=$(hostname) resume=${RESUME:-fresh} ====="
