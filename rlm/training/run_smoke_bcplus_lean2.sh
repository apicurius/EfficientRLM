#!/bin/bash
# BC+-only LEAN2 window-aligned 60-step smoke on the EfficientRLM tree (group_size 8,
...
CFG=$EFF/rlm/training/configs/smoke-qwen3-30b-bcplus-lean2-winaligned-60step.toml
mkdir -p $EFF/outputs
...
echo "[smoke] ===== START bcplus-lean2-winaligned-60step $(date +%F_%H:%M:%S) host=$(hostname) resume=${RESUME:-fresh} ====="
